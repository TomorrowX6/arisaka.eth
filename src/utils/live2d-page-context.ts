...
export type PageSnapshot = { page: PublicPageContext | null; key: string; protected: boolean; article: boolean };
...
export function readPublicPage(): PageSnapshot {
	...
	if (!main) return { page: null, key: path, protected: false, article: false };
	if (main.querySelector("[data-encrypted-article], [data-decrypted-article]")) {
		return { page: null, key: path, protected: true, article: true };
	}
	// About and Friends also contain Markdown; only post pages have this wrapper.
	const article = main.querySelector("#post-container");
	if (!article) return { page: null, key: path, protected: false, article: false };
	const source = article.querySelector(".custom-md") ?? article;
	...
	const title = (article.querySelector('[data-pagefind-meta="title"], h1')?.textContent?.trim() || document.title).slice(0, 160);
	if (!text) return { page: null, key: path, protected: false, article: true };
	const page = { title, path, text };
	return { page, key: JSON.stringify(page), protected: false, article: true };
}
