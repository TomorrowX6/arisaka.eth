...
### 自带GC(垃圾回收)
Go 程序创建对象后，不需要手动释放。Go 运行时会检查哪些内存已经没有人使用，然后自动回收。  
相较于Rust的GC(所有权,借用,生命周期)，学习成本低。

### 代码风格统一
gofmt会自动整理缩进和代码格式，团队成员写出来的代码风格基本一致。  
降低了屎山代码概率。

### 并发编程简单
Go 内置了 goroutine，可以方便地同时执行多个任务。  

<br>

# Go语言基础与速查表

引用自[Quick Reference](https://book.rmb.run/docs/golang.html)
