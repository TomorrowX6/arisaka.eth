...
export function createSystem(controls) {
  ...
  async function openFile(input, mode) {
    ...
    const kind = mode === 'hex' ? 'hex' : /(?:callgrind|cachegrind)(?:\.out)?(?:\.\d+)?$|\.callgrind$/i.test(path) ? 'profiler' : /\.(zip|tar|gz|tgz)$/i.test(path) ? 'archive'
      : /(?:\.trs|\.signal\.(?:csv|tsv))$/i.test(path) ? 'signal' : /\.bpf(?:\.o)?$/i.test(path) ? 'bpf' : /\.vcd$/i.test(path) ? 'logic' : /\.(der|cer|cbor|cborseq)$/i.test(path) ? 'structure' : /\.(pcap|pcapng|cap)$/i.test(path) ? 'packets' : /\.(db|sqlite|sqlite3)$/i.test(path) ? 'database'
