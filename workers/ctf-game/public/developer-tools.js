...
export function createDeveloperTools(controls) {
  ...
  function createProfiler() {
    ...
    return { open, reset() { profile = null; path = ''; selected = ''; $('#profiler-path').textContent = ''; $('#profiler-function').textContent = ''; $('#profiler-calls').replaceChildren(); $('#profiler-source').replaceChildren(); $('#profiler-status').textContent = ''; $('#profiler-export').disabled = true; $('#profiler-event').replaceChildren(new Option('成本事件', '')); $('#profiler-event').disabled = true; render(); } };
  }
